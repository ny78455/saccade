"""Run this script once to append describe_batch() to gemma4.py."""
import pathlib, textwrap

TARGET = pathlib.Path(__file__).parent / "gemma4.py"

BATCH_CODE = textwrap.dedent('''
    ...
    def _supports_batching() -> bool:
        ...
        depends on the specific processor implementation.  Runs a 2-item dummy probe.
        Result is cached after the first call.

        Returns True if batching supported; False otherwise.
        """
    ...
    def describe_batch(
        ...
        Context dicts: {action_label: str, dialogue_text: Optional[str], scene_labels: Optional[list]}
        ...
        try:
            ...
            for img, ctx in zip(images, contexts):
                ...
                if img is None:
                    ...
                    continue
                label_str = ", ".join(
                    \'"\' + l + \'"\' for l in labels if l != "unknown"
                )
                dialogue_text = ctx.get("dialogue_text")
                action_label  = ctx.get("action_label", "static")
                dlg = (
                    \' Dialogue spoken: "\' + dialogue_text[:80]
                    + ("..." if len(dialogue_text) > 80 else \'\') + \'"\\'
                    if dialogue_text else ""
                )
                prompt = (
                    "Look at this image and respond in EXACTLY this format (two lines):\\n"
                    "SCENE: <one label from: " + label_str + ">\\n"
                    "CAPTION: <one concise sentence describing the action and setting>\\n"
                    "Context -- action: " + action_label + "." + dlg
                )
...
if "_supports_batching" in existing:
    print("Already contains _supports_batching -- skipping.")
else:
    TARGET.write_text(existing + BATCH_CODE, encoding="utf-8")
    print(f"Appended batch functions to {TARGET}")
